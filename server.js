...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'erro', message: e.message });
  }
});
...
app.post('/api/admin/login', async (req, res) => {
  ...
  if (error) return res.status(500).json({ success: false, message: 'Erro ao verificar credenciais: ' + error.message });
...
app.get('/api/admin/sales', requireAdminAuth, async (req, res) => {
  ...
  res.json({ success: true, data: (data||[]).map(s => ({ ...s, purchaseDate: s.purchased_at, pdfFile: s.pdf_file, ticketCode: s.ticket_code, eventName: s.event_name, customerName: s.customer_name, customerEmail: s.customer_email, totalPrice: parseFloat(s.total_price) })) });
});
...
app.post('/api/admin/events', requireAdminAuth, uploadEvent.fields([{ name: 'cover', maxCount: 1 }, { name: 'poster', maxCount: 1 }]), async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin.from('events').insert({
    ...
    description: description||'', cover: req.files?.cover?.[0]?.filename||null,
    poster: req.files?.poster?.[0]?.filename||null, status: 'approved', submitted_by_name: 'Administrador'
  }).select().single();
  if (error) return res.status(500).json({ success: false, message: error.message });
...
// ------------------------------------------------------------
// API SUBMISSÃO DE EVENTO
// ------------------------------------------------------------
app.post('/api/events/submit', requireAuth, uploadEvent.fields([{ name: 'cover', maxCount: 1 }, { name: 'poster', maxCount: 1 }]), async (req, res) => {
  const { name, date, time, location, price, category, totalSeats, emoji, description } = req.body;
  if (!name || !date || !time || !location || !price || !totalSeats)
    return res.status(400).json({ success: false, message: 'Preencha todos os campos obrigatórios.' });
  const { error } = await supabaseAdmin.from('events').insert({
    ...
    total_seats: parseInt(totalSeats), sold_seats: 0, emoji: emoji||'🎟️',
    description: description||'', cover: req.files?.cover?.[0]?.filename||null,
    poster: req.files?.poster?.[0]?.filename||null, status: 'pending',
...
app.post('/api/purchase', requireAuth, async (req, res) => {
  ...
  const { data: event, error: evErr } = await supabaseAdmin.from('events').select('*').eq('id', eventId).eq('status', 'approved').single();
  if (evErr || !event) return res.status(404).json({ success: false, message: 'Evento não encontrado.' });
  if (event.sold_seats + qty > event.total_seats) return res.status(400).json({ success: false, message: 'Lugares insuficientes.' });
  ...
  try {
    const qrCodeDataURL = await QRCode.toDataURL(validateUrl, { errorCorrectionLevel: 'H', margin: 1, color: { dark: '#1a1a2e', light: '#FFFFFF' }, width: 200 });
    const pdfFileName   = `bilhete-${ticketCode}.pdf`;
    await generateTicketPDF(path.join(__dirname, 'tickets', pdfFileName), { ticketCode, event, customerName: req.user.name, customerEmail: req.user.email, quantity: qty, purchaseDate, qrCodeDataURL });
    await supabaseAdmin.from('events').update({ sold_seats: event.sold_seats + qty }).eq('id', eventId);
    await supabaseAdmin.from('sales').insert({ ticket_code: ticketCode, event_id: eventId, event_name: event.name, customer_name: req.user.name, customer_email: req.user.email, user_id: req.user.id, quantity: qty, total_price: event.price * qty, pdf_file: pdfFileName, validated: false });
    res.json({ success: true, message: 'Bilhete gerado!', data: { ticketCode, pdfUrl: `/tickets/${pdfFileName}`, eventName: event.name, customerName: req.user.name, totalPrice: event.price * qty } });
  } catch(err) {
...
app.post('/api/validate', requireAuth, async (req, res) => {
  ...
  const { data: sale, error } = await supabaseAdmin.from('sales').select('*').eq('ticket_code', ticketCode).single();
  if (error || !sale) return res.json({ success: false, valid: false, message: 'Bilhete inválido ou inexistente.' });
  if (sale.validated) return res.json({ success: true, valid: false, message: 'Este bilhete já foi utilizado.', data: { ...sale, ticketCode: sale.ticket_code, eventName: sale.event_name, customerName: sale.customer_name, customerEmail: sale.customer_email } });
  await supabaseAdmin.from('sales').update({ validated: true, validated_at: new Date().toISOString(), validated_by: req.user.name }).eq('ticket_code', ticketCode);
  res.json({ success: true, valid: true, message: 'Bilhete válido! Acesso autorizado.', data: { ...sale, ticketCode: sale.ticket_code, eventName: sale.event_name, customerName: sale.customer_name, customerEmail: sale.customer_email, validatedBy: req.user.name } });
});
...
async function generateTicketPDF(outputPath, data) {
  return new Promise((resolve, reject) => {
    ...
    const iY=175;
    const df=(label,value,x,y)=>{ doc.fontSize(7).fillColor('#8888aa').font('Helvetica').text(label.toUpperCase(),x,y); doc.fontSize(9.5).fillColor('#ffffff').font('Helvetica-Bold').text(String(value),x,y+11); };
